...
def get_fontes_status():
    ...
    for r in conn.execute("""SELECT fonte, idioma, COUNT(*) as total,
        ...
        if r["fonte"].startswith("Google News/"):
            sync_key = "Noticias/Google News BR" if r["idioma"] == "pt" else "Noticias/Google News EN"
        else:
            sync_key = f"Noticias/{r['fonte']}"
        sync = conn.execute("SELECT status, created_at FROM sync_log WHERE fonte = ? ORDER BY created_at DESC LIMIT 1", (sync_key,)).fetchone()
        noticias.append({**dict(r), "ultima_coleta": sync["created_at"] if sync else None, "sync_status": sync["status"] if sync else "nunca"})
...
@app.post("/api/editorial/{fila_id}/voltar_revisao")
def editorial_voltar_revisao(fila_id: int):
    conn = get_conn()
    conn.execute("UPDATE noticias_fila SET status_editorial = 'revisao' WHERE id = ?", (fila_id,))
    conn.commit()
    conn.close()
    return {"status": "ok"}
...
@app.post("/api/editorial/{fila_id}/refazer")
async def editorial_refazer(fila_id: int, request: Request):
    import threading
    body = await request.json() if request.headers.get("content-type", "").startswith("application/json") else {}
    instrucao = body.get("instrucao", "")

    # Se instrução vazia ou genérica, reprocessa do zero
    instrucao_limpa = instrucao.strip().lower()
    if not instrucao_limpa or instrucao_limpa in ("reescrever", "reescreva", "refazer"):
        def _run():
            from scrapers.processar_noticias import reprocessar_noticia
            reprocessar_noticia(fila_id)
        threading.Thread(target=_run, daemon=True).start()
        return {"ok": True, "status": "reprocessando"}

    # Com instrução específica, reprocessa com a instrução
    def _run_instrucao():
        from scrapers.processar_noticias import reprocessar_noticia
        reprocessar_noticia(fila_id, instrucao=instrucao)
    threading.Thread(target=_run_instrucao, daemon=True).start()
    return {"ok": True, "status": "reprocessando"}
...
def gerar_img_noticia(noticia_id: int):
    ...
    conn = get_conn()
    conn.execute(
        "UPDATE noticias_fila SET imagem_status = 'pendente', imagem_url = NULL WHERE id = ?",
        (noticia_id,),
    )
    conn.commit()
    conn.close()
    ...
    def _run():
        ...
        from scrapers.gerar_imagem import buscar_e_salvar_pexels
        result = buscar_e_salvar_pexels(noticia_id)
        if result is None:
            conn2 = get_conn()
            conn2.execute("UPDATE noticias_fila SET imagem_status = 'erro' WHERE id = ?", (noticia_id,))
            conn2.commit()
            conn2.close()
...
@app.post("/api/imagens/refazer/noticia/{noticia_id}")
async def refazer_imagem_noticia(noticia_id: int, request: Request):
    import threading
    body = await request.json()

    conn = get_conn()
    conn.execute(
        "UPDATE noticias_fila SET imagem_status = 'gerando', imagem_url = NULL WHERE id = ?",
        (noticia_id,),
    )
    conn.commit()
    conn.close()
    ...
    threading.Thread(target=_pexels, daemon=True).start()
    return {"ok": True, "status": "buscando_pexels"}


@app.post("/api/pipeline/rodar")
async def pipeline_rodar(request: Request):
    import threading
    body = await request.json() if request.headers.get("content-type", "").startswith("application/json") else {}
    limite = body.get("limite", 5)

    def _run():
        try:
            from scrapers.noticias import coletar
            from scrapers.processar_noticias import processar
            coletar()
            processar(limite=limite)
        except Exception as e:
